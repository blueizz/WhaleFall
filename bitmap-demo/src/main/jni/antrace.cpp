...
static void calc_dimensions(imginfo_t *imginfo, potrace_path_t *plist) {
    ...
    if (imginfo->width == UNDEF && imginfo->height != UNDEF) {
        imginfo->width =
                imginfo->height / imginfo->trans.bb[1] * imginfo->trans.bb[0] / info.stretch;
    } else if (imginfo->width != UNDEF && imginfo->height == UNDEF) {
        imginfo->height =
                imginfo->width / imginfo->trans.bb[0] * imginfo->trans.bb[1] * info.stretch;
    }
    ...
    if (info.backend->fixed) {
        if (imginfo->lmar == UNDEF && imginfo->rmar == UNDEF) {
            ...
            imginfo->lmar +=
                    (info.paperwidth - imginfo->trans.bb[0] - imginfo->lmar - imginfo->rmar) / 2;
        }
        if (imginfo->bmar == UNDEF && imginfo->tmar == UNDEF) {
            ...
            imginfo->bmar +=
                    (info.paperheight - imginfo->trans.bb[1] - imginfo->bmar - imginfo->tmar) / 2;
        }
    } else {
...
jobject createdPoint(JNIEnv *env, dpoint_t *pt) {
    jobject ret;
    jclass cls = env->FindClass("com/blueizz/bitmap/antrace/bean/Dpoint");
    jmethodID constructor = env->GetMethodID(cls, "<init>", "()V");
    ret = env->NewObject(cls, constructor);
...
jobject createCurve(JNIEnv *env, potrace_curve_t *c) {
    jobject ret;
    jclass crvcls = env->FindClass("com/blueizz/bitmap/antrace/bean/Curve");
    ...
    jclass dptcls = env->FindClass("com/blueizz/bitmap/antrace/bean/Dpoint");
    // one dimension array
    jclass d1cls = env->FindClass("[Lcom/blueizz/bitmap/antrace/bean/Dpoint;");
    ...
    fid = env->GetFieldID(crvcls, "c", "[[Lcom/blueizz/bitmap/antrace/bean/Dpoint;");
    env->SetObjectField(ret, fid, arr);
...
jobject createPath(JNIEnv *env, potrace_path_t *path) {
    jobject ret = 0;
    jclass cls = env->FindClass("com/blueizz/bitmap/antrace/bean/Path");
    jmethodID constructor = env->GetMethodID(cls, "<init>", "()V");
    ret = env->NewObject(cls, constructor);
    ...
    jfieldID fid = env->GetFieldID(cls, "curve", "Lcom/blueizz/bitmap/antrace/bean/Curve;");
    env->SetObjectField(ret, fid, jcurve);
    env->DeleteLocalRef(jcurve);
    env->DeleteLocalRef(cls);
...
Java_com_blueizz_bitmap_antrace_Utils_traceImage(JNIEnv *env, jobject thiz, jobject bitmap) {
    ...
    for (potrace_path_t *n = start; n != 0; n = n->next) {
        ...
        if (prev != 0) {
            jfieldID fid = env->GetFieldID(cls, "next",
                                           "Lcom/blueizz/bitmap/antrace/bean/Path;");
...
Java_com_blueizz_bitmap_antrace_Utils_threshold(JNIEnv *env, jobject thiz, jobject input, jint t,
                                                jobject output) {
...
JNIEXPORT void JNICALL
Java_com_blueizz_bitmap_antrace_Utils_unsharpMask(JNIEnv *env, jobject thiz, jobject input,
                                                     jobject output) {
    AndroidBitmapInfo inputInfo;
    AndroidBitmapInfo outputInfo;
    void *src_pixels = 0;
...
void histogramNormalize(void *src_pixels, void *dst_pixels, int w, int h, int stride,
                        int32_t histogram[]) {
...
JNIEXPORT void JNICALL
Java_com_blueizz_bitmap_antrace_Utils_grayScale(JNIEnv *env, jobject thiz, jobject input,
                                                   jobject output) {
    AndroidBitmapInfo inputInfo;
    AndroidBitmapInfo outputInfo;
    int ret = 0;
    ...
    histogramNormalize(src_start, dst_start, outputInfo.width, outputInfo.height, inputInfo.stride,
                       histogram);
...
Java_com_blueizz_bitmap_antrace_Utils_saveSVG(JNIEnv *env, jobject thiz, jstring path, int w,
                                                 int h) {
    return saveToFile(env, thiz, path, w, h, "svg");
}

JNIEXPORT jboolean JNICALL
Java_com_blueizz_bitmap_antrace_Utils_saveDXF(JNIEnv *env, jobject thiz, jstring path, int w,
                                                 int h) {
    return saveToFile(env, thiz, path, w, h, "dxf");
}

JNIEXPORT jboolean JNICALL
Java_com_blueizz_bitmap_antrace_Utils_savePDF(JNIEnv *env, jobject thiz, jstring path, int w,
                                                 int h) {
    return saveToFile(env, thiz, path, w, h, "pdfpage");
}

JNIEXPORT void JNICALL Java_com_blueizz_bitmap_antrace_Utils_clearState(JNIEnv *env) {
    if (s_state != NULL) {
        potrace_state_free(s_state);
        s_state = NULL;
    }
